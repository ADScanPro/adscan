...
class PostDAShell(Protocol):
    ...
    audit_post_da_bh_refresh_policy: str
    audit_post_da_bh_refresh_max_cycles: int
...
def _prompt_opt_in_privileged_refresh(
    *,
    ...
    Args:
        ...
        policy: Configured refresh policy.
        max_cycles: Max refresh cycles (used for UX hints).
    ...
    info_lines = [
        ...
        "",
        "[bold]Tradeoffs[/bold]",
        f"- Extra runtime (up to {max_cycles} cycle(s) with policy={policy})",
...
def get_audit_post_da_bh_refresh_policy(shell: PostDAShell) -> str:
    """Return post-DA BloodHound refresh policy for audit mode."""
    raw_policy = (
        str(getattr(shell, "audit_post_da_bh_refresh_policy", "once") or "once")
        .strip()
        .lower()
    )
    if raw_policy in {"off", "once", "until_stable"}:
        return raw_policy
    return "once"


def get_audit_post_da_bh_refresh_max_cycles(shell: PostDAShell) -> int:
    """Return maximum post-DA refresh cycles for audit mode."""
    raw_cycles = getattr(shell, "audit_post_da_bh_refresh_max_cycles", 1)
    try:
        cycles = int(raw_cycles)
    except (TypeError, ValueError):
        cycles = 1
    return max(1, min(cycles, 10))
...
def run_audit_post_da_bloodhound_refresh(
    ...
    policy = get_audit_post_da_bh_refresh_policy(shell)
    if policy == "off":
        return
    ...
    marked_domain = mark_sensitive(domain, "domain")
    marked_user = mark_sensitive(username, "user")
    max_cycles = get_audit_post_da_bh_refresh_max_cycles(shell)
    previous_total, previous_theoretical = collect_tier0_path_counts(shell, domain)
    cycles_done = int(state.get("bh_da_refresh_cycles", 0) or 0)
    stable_reached = bool(state.get("bh_da_refresh_stable_reached"))

    if policy == "until_stable" and stable_reached:
        print_info_debug(
            "[post-da] Skipping refresh because attack-path stability was previously reached."
        )
        return
    ...
    while cycles_done < max_cycles:
        upload_ok = True
        cycle_number = cycles_done + 1
        print_info(
            "Refreshing relationship-graph collection as "
            f"{marked_user} for {marked_domain} "
            f"(cycle {cycle_number}/{max_cycles}, policy={policy})."
        )
        ...
        state["bh_da_refresh_cycles"] = cycles_done
        ...
        state["bh_da_refresh_policy"] = policy
        state["bh_da_refresh_last_paths_to_tier0"] = current_total
        state["bh_da_refresh_last_theoretical"] = current_theoretical

        if policy != "until_stable":
            break

        stable_now = (
            current_total == previous_total
            and current_theoretical == previous_theoretical
        )
        if stable_now:
            state["bh_da_refresh_stable_reached"] = True
            print_info(
                "Post-DA graph refresh reached stable attack-path counts; stopping refresh cycles."
            )
            break

        previous_total, previous_theoretical = current_total, current_theoretical

    if policy == "until_stable" and cycles_done >= max_cycles:
        state["bh_da_refresh_max_cycles_reached"] = True
        print_warning(
            "Post-DA graph refresh reached the configured max cycles before stability."
        )
...
__all__ = [
    ...
    "get_audit_post_da_bh_refresh_policy",
